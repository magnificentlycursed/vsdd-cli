...
4. **PII redaction at boundaries.** PII fields are redacted at egress boundaries (logs, error messages, audit trails) and — the load-bearing surface here — at the **record-recording boundary**: the harness run records (transcripts) capture tool inputs (bash commands, file paths, file contents), a secret-carrying surface, so redaction/anonymization applies **before any value is recorded**, not at a downstream collector. PII/secret patterns in `.vsdd/registry/anonymization-patterns.yaml` (operator-extensible). Coordinate with Security (transcripts as a secret-carrying surface).
...
- Flags to **Security** when redaction must cover the run-record / transcript surface (redaction at the record-recording boundary; transcripts as a secret-carrying surface)
