...
4. **Manual-test preamble + checklist completeness.** the slice's manual-test checklist (`manual-tests/`) has the preamble (test_class + layer + target_artifact + tested_against + prerequisites + expected_outcomes + falsifiability_check) + checkbox items per behavior. Missing preamble fires `VSDD-E0018`; vague checkbox items fire `VSDD-W0080`.
...
9. **Governing-law testability — "authored is not exercised."** The conformance subsystem's own law — the sibling of "availability is not activation" (contract, Conformance at action time — Availability is not activation) — is a QE object: an authored control must emit a provable "it ran" trace, and the check that audits it must **fail loud on authored-but-never-exercised**. QE owns that this law is itself testable. Every requirement in the settled amendment (the **Verifiable conformance and efficiency** contract member; design rationale on the `verifiable-conformance-and-efficiency` knowledge page) must name a real falsifier, and every acceptance criterion must be **mechanically checkable** — a machine, not a human, decides pass/fail from the traces. A requirement whose falsifier cannot be constructed, or an acceptance criterion that needs human judgment to score, is a QE finding: it can only be convention, not a control. The control-effectiveness registry (the roster of authored controls paired with their expected-fire proof) is the coverage surface — every entry needs a negative case that would fail if the control silently stopped firing.
