...
9. **Oracle-forgeability probing.** Can the checked agent fabricate its own conformance evidence? Probe whether any control rests on an agent-writable record; the oracle must be harness-produced (transcripts the agent cannot author) + CI over server-synced state (the server-synced authorship basis, contract Trust boundaries), per the unforgeable-oracle control. Coordinate with Security on the conformance-oracle trust boundary.
