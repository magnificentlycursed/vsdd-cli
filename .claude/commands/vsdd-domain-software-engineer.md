...
These dimensions apply as-is when the built artifact is the harness's own conformance verifier or efficiency engine — pure functions over the traces, homed in `vsdd-core` with a thin binary command surface. The purity-boundary discipline (dimension 4) is load-bearing there: the verifier and engine read the records and must not write them or otherwise depend on process state.
