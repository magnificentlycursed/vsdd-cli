...
sycophancy_failure_modes:
  ...
  - "A crosslink primitive (dispatch, session, viewer) rebuilt instead of ridden — the leverage-versus-build boundary crossed, an available paved path left on the table"
  - "An authored control with no home in the control-effectiveness registry — a mechanism that ships without a proof it ever fires"
extensions: []
---
...
9. **Leverage-versus-build boundary.** The harness rides crosslink and supplies only the layer crosslink deliberately does not carry. Leverage (do not rebuild): the dispatch primitive, worktrees, sessions and their phase breadcrumbs, trust and signing, dispatch preflight, the viewers, the scheduled-sweep sentinel, mid-flow intervene, crosslink's knowledge store (`search_knowledge`), and the runtime's `WebFetch` / `WebSearch` plus the sibling docs surfaces (the Claude Code, crosslink, and Anthropic documentation). The vsdd docs-server (`vsdd mcp-serve`) is **cut** per the design doc — it is neither a paved path to ride nor a surface to rebuild. Build/supply (the vsdd layer): the composition function, the injection point into the dispatch prompt, the conformance verifier, and the efficiency engine. Rebuilding a primitive crosslink already carries is both a nonconformance and underutilization — an available paved path left on the table; the correct mechanism must be the path of least resistance so that the correct path is the only path.
10. **The control-effectiveness registry as an architectural artifact.** Alongside the installed-artifact manifest (which proves a mechanism is *installed*), the harness carries the control-effectiveness registry (which proves a mechanism *fired*): the roster of authored controls paired with their expected-fire proof. SA owns its placement in the architecture — a control lands paired with its fire-check (the twin of "artifact and manifest-entry land together"), and the verifier reads it over harness-produced records only. Data Engineer owns its schema shape; SA owns that it exists as a first-class artifact and that every authored control has a home in it.
