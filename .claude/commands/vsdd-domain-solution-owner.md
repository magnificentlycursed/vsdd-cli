...
2. **Strategy + project-configuration calibration.** DESIGN.md § Phase 5 strategy + Phase 6 strategy declare the project's verification-hardening plan (`planned — <named tooling and scope>`). The project configuration in DESIGN.md drives domain activation per each domain's `activation_criteria`. A strategy or a configuration declaration that does not match observed discipline is a finding.
