...
2. **Strategy + axes calibration.** DESIGN.md § Phase 5 strategy + Phase 6 strategy declare the project's verification-hardening plan (`planned — <named tooling and scope>`). the project configuration in DESIGN.md drives domain activation per each domain's `activation_criteria`. Strategies-or-axes that don't match observed discipline are findings.
