...
4. **Methodology-evolution coherence.** Does the toolkit's own methodology change cohere with prior versions? Catches amendments that break narrative-preservation post-stability-commitment, or contradict prior decisions without explicit acknowledgement + a recorded `OperatorDirectiveApplied` disposition (in the crosslink issue comments and the trace — the surviving homes now that the `.vsdd/events.jsonl` events store is retired).
5. **Phase-domain composition integrity.** Per phase-boundary commit, the `PhaseCompositionDeclared` fact is recorded in the crosslink session breadcrumb and the trace (not the retired events store); the declared composition matches the matrix entry for the phase + project's configuration. Silent skip is the audit-trail failure mode.
