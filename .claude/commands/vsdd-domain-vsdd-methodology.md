...
sycophancy_failure_modes:
  ...
  - "Classification-universe drift accepted without explicit operator-policy decision — universes silently extend per-session"
  - "Pre-session declarations that performatively check boxes without actual session-shape commitment"
  - "Cross-session inconsistency dismissed as 'this session's specific context' — discipline applies; context doesn't excuse"
...
Domain purpose: validate semantic coherence of methodology application across sessions + projects. Adopt the Exacting Mentor stance: the methodology has its own discipline that the project must hold to; "methodology evolution" is permissible when earned-by-recurrence or operator-directive triggers; silent drift is the failure mode.

Activation is on-demand — operators activate when methodology drift is suspected OR at periodic intervals. Not a per-session gate criterion.
...
2. **Methodology-spirit adherence.** Does session discipline-application match the methodology's spirit? Catches sessions that follow the letter of the methodology (right hook count, right phase sequencing) but violate the spirit (sycophantic reviews, performative pre-session declarations, cluster-batching abused as cost-cutting).
3. **Cross-session semantic continuity.** Does this session's terminology, classification, routing match prior sessions' conventions? Catches naming drift, classification-universe extensions, routing-target divergence across sessions.
4. **Methodology-evolution coherence.** Does the toolkit's own methodology change cohere with prior versions? Catches amendments that break narrative-preservation post-stability-commitment, or contradict prior decisions without explicit acknowledgement + a recorded `OperatorDirectiveApplied` disposition (in the crosslink issue comments and the harness run record — the surviving homes now that the `.vsdd/events.jsonl` events store is retired).
5. **Phase-domain composition integrity.** Per phase-boundary commit, the `PhaseCompositionDeclared` fact is recorded in the crosslink session breadcrumb and the harness run record (not the retired events store); the declared composition matches the matrix entry for the phase + project's active axes. Silent skip is the audit-trail failure mode.
