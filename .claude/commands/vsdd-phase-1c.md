...
- Names its manual-test checklist obligation where the slice ships operator-facing behavior (`manual-tests/`; the checklist itself is operator-authored at Phase 2a entry — the operator authors the oracle; no scaffold mechanism exists)
...
- Names its **Exit Signal pointer** (the `ExitSignaled` fact recorded in the trace and the crosslink session breadcrumb when the milestone closes)
