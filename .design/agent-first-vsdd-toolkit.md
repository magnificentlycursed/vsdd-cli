...
Revision: spec-review round 1 applied (44 findings, 4 cold lenses; dispositions filed as child issues of the tracking issue).
...
- Filing is evidence-gated: a finding carries checkable evidence at filing time, either a file-and-line citation that mdatron verifies against HEAD or a failing test where the surface allows one. A finding citing code absent from HEAD is rejected at filing.
...
Falsification: lifecycle state found in a review-log file that differs from the tracker; a closed finding without an evidence reference; an open finding without an owner; a filed finding whose citation fails HEAD verification.
...
- **Gates** — phase gates ship as commands runnable by `crosslink swarm gate`: red-gate checkout-and-run, unrouted-findings query, mutation floor, and fresh-container install-and-smoke via `crosslink kickoff --container`, mechanizing the fresh-system install check that has been satisfied once in project history, manually (Phase exit by gate).
