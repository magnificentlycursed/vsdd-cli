...
- The review config is a file (`.vsdd/config.yaml`) with layered loading modeled on crosslink's config: shipped presets, then project config, then local overrides. Presets are suggested starting points, each with an associated cost band (a preset's composition yields its priced bill of materials once the pricing function runs over the stamped counts). The operator customizes freely; the customization is a declared, git-tracked, schema-validated input. Fields: active domains, round budget, stop sensitivity, model tiers, mutation floor, cost bands.
...
- **Status detection** (Status requirement) — a fixture repo with crosslink initialized and no session started is reported by status; a mid-cycle fixture without milestones is reported; an artifact-versus-tracker disagreement fixture files the integrity finding without degrading the answer; a tracker-absent fixture and a tracker-malformed fixture each yield the degraded marking; a malformed state file yields a diagnostic, never a panic; the concurrent-boundary clean-merge-inconsistent fixture is caught by the state-consistency check.
- **Oracle provenance** (The operator authors the oracle) — a provenance audit over the fixture cycle's criteria, manual-test items, and fixture reference answers finds operator adoption recorded for each; a seeded agent-only criterion is flagged; a seeded agent-authored reference answer without recorded adoption is flagged; a seeded agent-asserted gate pass with no gate record is flagged.
...
- The composition function: (DESIGN.md surfaces, phase, review config) to the active domain set and dispatch shape. Properties: determinism (identical inputs, identical output), pair co-activation, pair separation, validator-differs-from-owner, and domain-narrowing (removing domains from the config never adds domains to the output; the monotonicity claim covers the active-domain field only).
...
- The phase-answer derivation: (state artifact, corroboration snapshot) to the structured phase answer plus the snapshot-scoped part of the process-integrity report. The corroboration snapshot is an explicitly-acquired materialized view — milestone states, round and finding counts, comment handles with resolution status precomputed — acquired in the effectful shell; the function is pure over an acquired snapshot and never acquires (the named refinement the phase-1b primer requires for takes-a-clock-shaped inputs). Integrity checks that cannot materialize into the snapshot — git-reference resolution, chassis compaction counts — run in the shell and join the report there. The snapshot schema is audited against the full Status integrity-query list at phase-1c data authoring, so every snapshot-scoped check's inputs (per-finding owner, validator, and evidence-reference presence included) are materialized fields rather than illustrative ones. Properties: determinism; degraded marking exactly when corroboration is absent or unusable (a present-but-disagreeing snapshot files the integrity finding without degrading, per the phase-state resolution).
- The phase enumeration and action vocabulary are data consumed by these functions, never computed.

Effectful shell (integration-tested, not proof territory): init deployment and drift refusal, hook wrappers, tracker and ledger IO, context generation IO, gate command execution.
...
- Manual by design, each with a named owner: methodology-rewrite verification (operator read plus one external cold reader), milestone conversion and ratification acts (operator), oracle authorship (operator, per The operator authors the oracle), and the per-layer manual-test checklists at build time (director tests, per canon and the scorecard's crown-jewel evidence).
...
- Fuzz testing: the config, state, registry, and tracker-payload parsers (adopter-edited YAML and tracker-sourced data; the registry is adopter-authored and also governed — see Trust boundaries); each repo's design owns its own fuzz surface, per the Phase 5 strategy.
...
Trust boundaries:
- Adopter-edited files: `.vsdd/config.yaml`, `.vsdd/state.yaml`, and the vocabulary registry (adopter-authored, and also routed and governed as a generated-context source — one artifact, both natures) — schema-validated at read by vsdd during bootstrap, by mdatron's conformance families once landed; malformed input yields a diagnostic, never a panic (mirroring the engine's contract).
