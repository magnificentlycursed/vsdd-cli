...
**Purpose:** Per-domain review-log entry within a Phase 3 session. The canonical audit-trail artifact.
...
review_number: <u32>             # session review number; deterministic per session
...
**Validation mode:** Frontmatter-based (per-issue frontmatter block within review-entry body).
...
blocked_by: [<finding_id>...]    # cross-issue dependencies
...
source: <SourceKind enum>        # inherits from Review entry; redeclaration per-issue for cross-source mixes
routing:
...
phase: <Phase enum>
version: <semver>
frequency: <string>              # per-milestone | per-project | per-session | etc.
...
**Purpose:** Per-milestone manual test artifact at `manual-tests/layer-N.md` OR per-binary at `manual-tests/<binary>.md` OR error-catalog fixtures at `manual-tests/error-catalog/<code>/{should-fire,should-not-fire}/`.
...
- If `test_class: per-milestone`, `layer` MUST be declared
...
required_fields:
  - scope                          # one-line milestone scope
  ...
  - exit_signal_pointer            # required when milestone closes
excluded_fields:
