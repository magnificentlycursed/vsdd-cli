...
- **Dispatch interception and consent surfaces (moved here from the contract, 2026-09-18).** vsdd's preventive controls hold only at the interception points it sits on — `vsdd dispatch`, crosslink's kickoff/swarm preflight, and the git/CI operations it gates. The runtime's native spawn primitive (the `Task` / `Agent` tool) bypasses all of them: a dispatch through it is caught only by the post-hoc trace audit (detective), and making it preventive needs a harness-level `PreToolUse` deny hook that intercepts the spawn and redirects to the golden path — an outer-harness dependency, carried in the exception register (`agent-count-interceptor-unbuilt`). The folder-trust dialog is TTY-gated and fires on any untrusted fresh worktree path; only the harness's own trust acceptance (`hasTrustDialogAccepted`) clears it, and no CLI-reachable headless grant exists — so the local/tmux path stalls headless while a container (`docker run -d`, no TTY) proceeds; permission-bypass flags clear only the bypass-permissions layer and are not adopted. The lifecycle index `journal.jsonl` (one line per agent lifecycle event) is harness-produced but is neither the conformance oracle nor an input to the cost-and-efficiency report; only the per-agent transcripts under the harness's project directory are read as the trace. On the Workflow path a per-run token budget directive is a hard ceiling (agent launches past it fail) — the fallback path's per-agent cap while the crosslink vehicle's `--budget-usd` is the target vehicle's; an agent-count ceiling exists on neither.
