...
cannot_run_predicate:
  command_binding: "the repo's declared test command at workspace scope — the same command the crosslink's gate resolves (crosslink hook-config agent_test_commands at this estate; the adopter's declared equivalent elsewhere)"
